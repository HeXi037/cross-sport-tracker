...
type FavouritePairingOption = {
  key: string;
  label: string;
  count: number;
};

type PlayerOptionGroup = {
  meOption: Player[];
  recentOptions: Player[];
  remaining: Player[];
};

type Props = {
  sportCopy: { playersHint?: string };
  isAnonymous: boolean;
  playerPreferences: { lastSelection: IdMap | null };
  handleApplyLastMatch: () => void;
  selectedPairingKey: string;
  setSelectedPairingKey: Dispatch<SetStateAction<string>>;
  ...
  duplicateHintId: string;
  isDuplicateSelection: (id: string) => boolean;
  filteredPlayerOptions: (key: keyof IdMap) => PlayerOptionGroup;
  doubles: boolean;
  isPadel: boolean;
...
export function PlayerSelectors({
  ...
  return (
    <fieldset className="form-fieldset" disabled={isAnonymous}>
      ...
      {isPadel && (
        <p className="form-hint" id={duplicateHintId} role="alert" aria-live="polite">
          Avoid selecting the same player twice.
        </p>
      )}
      {duplicatePlayerNames.length > 0 && (
        ...
        >
          Duplicate player names returned: {duplicatePlayerNames.join(", ")}. Each player name must be
