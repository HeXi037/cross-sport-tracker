...
interface BowlingFieldError {
  frameIndex: number;
  rollIndex: number | null;
}

interface BowlingEntriesSectionProps {
  bowlingEntries: BowlingEntry[];
  bowlingValidationErrors: (string | null)[];
  bowlingFieldErrors: (BowlingFieldError | null)[];
  ...
  previewBowlingInput: (frames: string[][]) => {
    total: number | null;
    frameTotals: (number | null)[];
  };
  getBowlingPlayerLabel: (entry: BowlingEntry, index: number, players: { id: string; name: string }[]) => string;
  getBowlingFramePinSum: (frame: string[]) => number;
  getBowlingInputKey: (entryIndex: number, frameIndex: number, rollIndex: number) => string;
  isBowlingRollEnabled: (frames: string[][], frameIndex: number, rollIndex: number) => boolean;
  buildBowlingRollLabel: (playerLabel: string, frame: number, roll: number) => string;
  handleBowlingPlayerChange: (index: number, value: string) => void;
  handleRemoveBowlingPlayer: (index: number) => void;
                    ...
                    >
                      ...
                      <div className={`bowling-rolls bowling-rolls--${frame.length}`}>
                        {frame.map((roll, rollIdx) => {
                          ...
                          const rollLabel = buildBowlingRollLabel(
                            playerLabel,
                            frameIdx + 1,
                            rollIdx + 1,
                          );
                          return (
                            <div key={rollIdx} className="bowling-roll-field">
                              ...
                              <div
                                ...
                                role="group"
                                aria-label={`${playerLabel} frame ${frameIdx + 1} roll ${
                                  rollIdx + 1
                                } shortcuts`}
                              >
                                <button
